...
<!--    </div>-->
    <div>
        ...
        <div class="slidecontainer" id="slider">
            <input onmouseup="{rectangle.animateAgain}" type="range" min="1" max="200" value="100" class="slider" style="width:950px" ref="mySlider">
        </div>
    </div>
    <script>
        var self = this;
        ...
        self.MovingDisplay = function (colours, mirroring, launchingType, stickAndHole, squareDimensions, canvas, slider=null, speed, facesBool) {
            ...
            // - display.flashOn worries about all squares being hidden, not just A
            ...
            display.setUp = function () {
                ...
                // overwrite "move At" instructions
                display.squareList[0].moveAt = Math.floor(Math.random() * display.squareList[1].duration*3) + 1; // returns a random integer from 1 to 3*duration;
                display.squareList[1].moveAt = Math.floor(Math.random() * display.squareList[1].duration*3) + 1; // returns a random integer from 1 to 3*duration;
                display.squareList[2].moveAt = Math.floor(Math.random() * display.squareList[1].duration*3) + 1; // returns a random integer from 1 to 3*duration;
            };
            ...
            display.setTimeouts = function (startInstructions = null) {
                ...
                if (display.showFlash) {
                    var flashTime = animationSpace / 200 * display.slider.value + startAt/2;
            ...
            display.flashOn = function () {
                if (display.showFlash === true) {
                    if (display.flashState === false) {
                        ...
                        if (display.squareList[2].colourName === "hidden") {
                            display.squareList[2].colour = "#000000";
                            display.squareList[2].drawMe(display.canvas);
                        }
                    } else {
                        ...
                        if (display.squareList[2].colourName === "hidden") {
                            display.squareList[2].colour = "#FFFFFF";
                            display.squareList[2].drawMe(display.canvas);
                        }
                    }
                }
        ...
        self.onInit = function () {
            self.rectangle = new self.MovingDisplay(["hidden", "blue", "hidden"], false, "canonical", false, [50, 50], self.refs.myCanvas, self.refs.mySlider, 0.3, false);
        };
        ...
        self.canLeave = function () {
            ...
            if ((self.currentMoment === 0 && self.refs.mySlider.value < 10)||
                (self.currentMoment === 1 && self.refs.mySlider.value > 190))
            {
                self.currentMoment++;
